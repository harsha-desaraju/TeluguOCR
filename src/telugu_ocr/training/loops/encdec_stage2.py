...
class DecoderTransformerBlock(nn.Module):
    """Transformer block; cross-attention (with a tanh gate) is optional.

    The cross-attention branch is gated: x = x + tanh(cross_attn_gate) * cross_out. In
    stage-1 the gate was zero-initialised (Flamingo-style) to phase the image in; here in
    stage-2 the gate is a normal trained parameter whose value is LOADED from the stage-1
    checkpoint, so the model starts exactly where stage-1 left off and keeps training it
    end-to-end. The constructor init below is only a placeholder — the stage-1 load
    overwrites it."""
    ...
    def __init__(self, config: GPTConfig, use_cross_attention: bool = True):
        ...
        if self.use_cross_attention:
            ...
            # Placeholder init (overwritten by the stage-1 checkpoint). NOT zero-init: these
            # are already-trained layers, so there is no init-time gating to phase in.
...
class EncoderDecoder(nn.Module):
    ...
    def __init__(self, encoder_config: CTCEncoderConfig, decoder_config: GPTConfig, pad_index: int,
                 ctc_loss_weight: float = CTC_LOSS_WEIGHT):
        super().__init__()
        ...
        # STAGE-2 (full end-to-end): the encoder trains too, so it MUST receive
        # gradients — the forward is NOT wrapped in torch.no_grad(). (In stage-1 this
        # was True to keep the frozen encoder out of the graph.)
        self.encoder_no_grad = False

        # Weight on the CTC auxiliary term in the combined loss (see CTC_LOSS_WEIGHT).
        self.ctc_loss_weight = ctc_loss_weight
        # Latest per-batch loss components, stashed for logging (see EncoderDecoderTrainer.log).
        self._ce_loss = None
        self._ctc_loss = None
    ...
    def forward(self, pixel_values, input_ids, input_lengths=None, text_padding_mask=None,
        ...
        # STAGE-2: encoder trains end-to-end (encoder_no_grad=False) so gradients flow.
        enc_ctx = torch.no_grad() if self.encoder_no_grad else contextlib.nullcontext()
        with enc_ctx:
            # Raw encoder frame features (encoder width, e.g. 384) BEFORE the bridge. The
            # CTC head consumes these; the decoder cross-attention consumes the bridged ones.
            encoder_output, key_padding_mask = self.encoder_model.encode(pixel_values, input_lengths)
        bridged = self.enc_to_dec(encoder_output)                # (B, T, dec_embed_dim); bridge is trainable
        ...
        if ctc_labels is not None and self.ctc_loss_weight > 0:
            # CTC head over the raw (pre-bridge) frame features -> (B, T, C=vocab+1),
            # same class/blank space as the standalone CTC encoder. log_softmax in fp32
            # for CTC numerical stability under fp16 autocast.
