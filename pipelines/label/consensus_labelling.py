...
class OCREngine:
    """Base class. `device_kind` decides scheduling: 'gpu' engines are serialized against
    each other, 'cpu' engines all run concurrently.

    `run` returns either a list of texts, or a (texts, scores) pair when the engine exposes
    a per-line confidence. Both are accepted so an engine that has no notion of confidence
    does not have to fake one.
    """
    ...
    def _tick(self, k: int = 1) -> None:
        """Report k finished images. Safe to call from worker threads, and a no-op when no
        progress sink is attached, so engines can call it unconditionally."""
        cb = self._on_items
        if cb is not None:
            cb(k)
...
class TesseractEngine(OCREngine):
    """Tesseract via pytesseract, one thread per in-flight crop.

    Two settings matter for line crops specifically:
      ...
      * upscaling. Tesseract wants roughly 30-35px of x-height; a 64px line crop is at the
        bottom of its comfortable range, and a 2x cubic upscale measurably helps.
    """

    name = "tesseract"
    device_kind = "cpu"

    def __init__(self, lang: str = "tel+eng", psm: int = 7, oem: int = 1,
